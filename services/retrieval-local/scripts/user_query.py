...
def select_book(books: List[str]) -> Optional[str]:
    ...
    while True:
        try:
            choice_str = input("Enter the book number (or 0 to cancel): ").strip()
            ...
            if 1 <= choice <= len(books):
                ...
                logger.info(f"User selected: {selected_book} - {selected_title}")
...
def embed_query(query: str, model: SentenceTransformer) -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.info(f"✓ Query embedding generated in {elapsed_time:.4f} seconds")
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if not client:
                print(f"❌ Failed to connect to {selected_book} database. Please try again.\n")
                continue
            
            collection = load_collection(client)
            if not collection:
                print(f"❌ Failed to load {selected_book} collection. Please try again.\n")
                continue
