...
OUTPUT_DIR = BASE_DIR / "outputs" / "new_embeddings_output"
...
def select_file(file_list: List[str]) -> str:
    ...
    try:
        choice = int(
            input("Enter the file number to process (or 0 to cancel): "))
        if choice == 0:
            return None
    ...
    try:
        ...
        for chunk in data['chunks']:
            ...
            if len(text) < MIN_TEXT_LENGTH:
                logger.warning(
                    f"Chunk {chunk_id} text too short ({len(text)} chars), skipping")
                continue

            if len(text) > MAX_TEXT_LENGTH:
                logger.warning(
                    f"Chunk {chunk_id} text too long ({len(text)} chars), truncating")
...
def initialize_chromadb(output_dir: Path) -> chromadb.Client:
    ...
    try:
        ...
        logger.info(
            f"Initialized ChromaDB with persist directory: {output_dir}")
        return client
...
def store_embeddings_in_chromadb(
    ...
    try:
        ...
        for batch_idx in range(total_batches):
            ...
            logger.info(
                f"Added batch {batch_idx + 1}/{total_batches} ({len(batch_ids)} embeddings)")

        logger.info(
            f"Stored {len(chunks)} embeddings in collection: {collection_name}")
...
def process_single_book(selected_file: str, book_number: int):
    ...
    try:
        ...
        if success:
            logger.info(
                f"✓ Book-{book_number}: All embeddings stored successfully")
        else:
    ...
    try:
        ...
        while True:
            ...
            if selected_file:
                ...
                logger.info(
                    f"Processing {selected_file} (Book {book_number})...")
